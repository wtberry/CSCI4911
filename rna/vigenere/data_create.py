...
def translateKeyTexts(keys, msgs, mode="encrypt"):
    ...
    for key, in_msg in keytext:
        ...
        d = [key,in_msg,out_msg] if mode=="encrypt" else [key,out_msg,in_msg] if mode=="decrypt" else []
...
i_keys = [w.upper() for w in words if 3 <= len(w) <= 8]
random.shuffle(i_keys)
train_keys = i_keys[:1000]
test_keys = i_keys[1000:2000]
key_file.close()
...
train_data = translateKeyTexts(train_keys,plaintexts)
