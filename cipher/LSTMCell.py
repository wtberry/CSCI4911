...
num_classes = 27
input_size = 28
hidden_size = 64  # output from the LSTM. 5 to directly predict one-hot
...
def data_load(dtype, batch_size, N):
    ...
    return inputs_cipher, labels_plain
...
class LSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        hx = Variable(torch.randn(batch_size, input_size))
        cx = Variable(torch.randn(batch_size, input_size))
...
LOG_DIR= 'Key_cipher_indicator'+'Layer_'+ str(num_layers)+'_lr_'+str(lr)+'_epoch_'+str(num_iter)
...
inputs_cipher_test, labels_plain_test = data_load('test', batch_size, N)

# Train the model
for epoch in range(num_iter):

    # loading the data using the function, training data
    inputs_cipher, labels_plain = data_load('train', batch_size, N)
...
prediction = lstm(inputs_cipher_test)
