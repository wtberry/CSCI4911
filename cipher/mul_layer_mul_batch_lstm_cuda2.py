...
torch.cuda.manual_seed(777)  # reproducibility putting cuda here works
torch.set_default_tensor_type('torch.cuda.FloatTensor') #THIS DID MAGIC STUFF
...
plain = [[cipher.iloc[i].key + cipher.iloc[i].plantxt[:N]] for i in range(batch_size)] #500 67.978
# plain = [[("A" * len(cipher.iloc[i].key)) + cipher.iloc[i].plantxt[:N]] for i in range(batch_size)] #98.9 (3020)
print(plain)
...
inputs_cipher = Variable(torch.Tensor(cipher_one_hot))#Don't put cuda here
labels_plain = Variable(torch.cuda.LongTensor(y_plain))
...
class LSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # (num_layers * num_directions, batch hidden_size) for batch_first=True #IMPLEMENTING CUDA HERE IS DIFFICULT
...
lstm = LSTM(num_classes, input_size, hidden_size, num_layers).cuda() #TRYING TO PUT CUDA ON MODEL can be done by putting .cuda() at the end but other things need to change
print(lstm)
...
for epoch in range(num_iter):
    ...
    if (epoch+1)%10==0:
        ...
        _, idx = outputs.max(1)
        #idx = idx.data.numpy()[:sequence_length] # printing out only one sentence CUDA FAILS HERE
        idx = idx.data[:sequence_length]    # sequence at length of key + length of N
        # print(idx)
        idx_label = labels_plain[0].data
...
labels_plain_test = Variable(torch.cuda.LongTensor(y_plain_test))#Maybe CUDA works here
